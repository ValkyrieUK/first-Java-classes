...
public class CollectionsTest {
	public static void main(String[] args) throws DodgyNameException {
		...
		// PART 2
		// Set<Account> accounts = new TreeSet<Account>(new
		// AccountComparator());

		double[] amounts = { 23, 5444, 2, 345, 34 };
		String[] names = { "Brad", "Konstantinos", "Mannan", "Spoc", "Thor" };

		// initialise the array of accounts contents to contain account objects
		Account current;
		for (int i = 0; i < 5; i++) {
			current = new CurrentAccount(names[i], amounts[i]);
		...
		while (iter.hasNext()) {
			...
			System.out.println("The name is " + nextAccount.getAccountName());
